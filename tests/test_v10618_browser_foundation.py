...
class BrowserFoundationTest(unittest.TestCase):
 ...
 def test_google_chrome_process_identity(self):
  ...
  with patch('vibrapilot.browser_diagnostics.collect_windows_browser_process',return_value={'status':'found','profile_path':r'C:\Profiles\slot_1','pid':1,'executable_path':r'C:\Program Files\Google\Chrome\Application\chrome.exe','command_line':'chrome'}), patch('vibrapilot.browser_diagnostics.collect_cdp_browser_metadata',return_value={'product':'Chrome/140.0'}), patch('vibrapilot.browser_diagnostics.collect_page_environment',return_value={'webdriver':True}):
  ...
  self.assertEqual(r['actual']['engine'],'google_chrome'); self.assertEqual(r['actual']['engine_evidence'],'confirmed_by_windows_process_path')
